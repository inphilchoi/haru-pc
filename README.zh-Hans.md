...
**不想使用我们的中继？** 安装时选择 **直接连接（自托管）**，或运行 `haru-pc relay off`。这样在同一个 Wi-Fi 下手机直接连到电脑；在外面可以用你自己的 Tailscale（`haru-pc remote on`），或自己运营的中继（`haru-pc relay url wss://…`）。详见 [docs/self-host-relay.md](docs/self-host-relay.md)。
